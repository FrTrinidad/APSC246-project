        ...
        if GPIO_AVAILABLE:
            ...
            for action, btn in self.gpio_buttons:
                ...
                if btn.is_pressed:
                    ...
                    self.gpio_pressed.add(action)
                else:
                    # If the button is not pressed we remove it from the pressed set
                    if action in self.gpio_pressed:
                        self.pressed_actions.discard(action)
                        self.gpio_pressed.discard(action)
                ...
                if ser.in_waiting > 0:
                    ...
                    if line and '|' in line:
                        ...
                        if len(parts) == 3:
                            ...
                            if x < 300:
                                print("down")
                                if "down" not in self.pressed_actions:
                                    self.just_pressed.add ("down")
                                self.pressed_actions.add("down")
                                self.gpio_pressed.add("down")
                            elif x > 700:
                                if "up" not in self.pressed_actions:
                                    self.just_pressed.add ("up")
                                self.pressed_actions.add("up")
                                self.gpio_pressed.add("up")
                            else:
                                if "up" in self.gpio_pressed:
                                    self.pressed_actions.discard("up")
                                    self.gpio_pressed.discard("up")
                                if "down" in self.gpio_pressed:
                                    self.pressed_actions.discard("down")
                                    self.gpio_pressed.discard("down")
                            
                            if y < 300:
                                if "left" not in self.pressed_actions:
                                    self.just_pressed.add ("left")
                                self.pressed_actions.add("left")
                                self.gpio_pressed.add("left")
                            elif y > 700:
                                if "up" not in self.pressed_actions:
                                    self.just_pressed.add ("right")
                                self.pressed_actions.add("right")
                                self.gpio_pressed.add("right")
                            else:
                                if "left" in self.gpio_pressed:
                                    self.pressed_actions.discard("left")
                                    self.gpio_pressed.discard("left")
                                if "right" in self.gpio_pressed:
                                    self.pressed_actions.discard("right")
                                    self.gpio_pressed.discard("right")
                            
                            if btn == 1:
                                if "confirm" not in self.pressed_actions:
                                    self.just_pressed.add ("confirm")
                                self.pressed_actions.add("confirm")
                                self.gpio_pressed.add("confirm")
                            else:
                                if "confirm" in self.gpio_pressed:
                                    self.pressed_actions.discard("confirm")
                                    self.gpio_pressed.discard("confirm")
            except:
                pass
